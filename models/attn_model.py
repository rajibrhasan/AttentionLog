...
class AttentionModel(Model):
    ...
    def prefill_inference(self, instruction, data):
        """Single forward pass (no generation). Returns full attention maps from prefill."""
        messages = [
            {"role": "system", "content": instruction},
            {"role": "user", "content": "Data: " + data}
        ]
        ...
        model_inputs = self.tokenizer(
            [text], return_tensors="pt", truncation=True, max_length=2048).to(self.model.device)
        ...
        attention_maps = [attn.detach().cpu().half() for attn in output['attentions']]
        attention_maps = [torch.nan_to_num(attn, nan=0.0) for attn in attention_maps]

        return attention_maps, data_range
