...
def parse_spirit_line(line):
    """Parse a single Spirit log line.

    Spirit format (same family as BGL/Thunderbird):
...
def load_spirit_sampled(data_path, n_normal=500, n_anomaly=500, seed=42):
    """Load Spirit data via reservoir sampling (single pass, memory-efficient).

    Args:
        data_path: Path to Spirit log file (e.g., spirit2.log).
...
def load_spirit_split(data_path, n_train_normal=100, n_train_anomaly=100,
                      n_test_normal=500, n_test_anomaly=500, seed=42):
    """Load Spirit data with train/test split via reservoir sampling.
