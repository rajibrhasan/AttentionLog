...
def parse_thunderbird_line(line):
    """Parse a single Thunderbird log line.

    Thunderbird format:
        <Label> <Timestamp> <Date> <Host> <Time> <HostRepeat> <Process[PID]>: <Content>
    Label is "-" for normal, an alert category tag for anomaly.

    The content starts after the process/PID field. We split on the first
    "]: " or ": " following the process info to separate metadata from content.
...
def load_thunderbird_sampled(data_path, n_normal=500, n_anomaly=500, seed=42):
    """Load Thunderbird data via reservoir sampling (single pass, memory-efficient).

    Args:
        data_path: Path to Thunderbird.log file.
    ...
    with open(data_path, "r", encoding="utf-8", errors="replace") as f:
        for line in f:
            parsed = parse_thunderbird_line(line)
            if parsed is None:
                continue
...
def load_thunderbird_split(data_path, n_train_normal=100, n_train_anomaly=100,
                           n_test_normal=500, n_test_anomaly=500, seed=42):
    """Load Thunderbird data with train/test split via reservoir sampling.
